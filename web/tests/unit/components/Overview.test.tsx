...
describe('Overview', () => {
  ...
  it('renders the hero, blurb and every overview section', () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /axios/i })).toBeInTheDocument();
    expect(screen.getByText(AXIOS.blurb)).toBeInTheDocument();
    // The install / quick-start / comparison / going-further / features sections.
    for (const suffix of ['install', 'quick', 'cmp', 'more', 'feat']) {
      expect(container.querySelector(`#${AXIOS.id}-${suffix}`)).not.toBeNull();
    }
    expect(screen.getByRole('heading', { name: 'Features' })).toBeInTheDocument();
    expect(container.querySelectorAll('ul.feat li').length).toBe(AXIOS.features.length);
  });
});
