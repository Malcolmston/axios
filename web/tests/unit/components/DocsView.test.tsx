...
const DOC_INDEX: DocIndex = {
  module: 'github.com/malcolmston/axios',
  packages: [
    {
      importPath: 'github.com/malcolmston/axios',
      name: 'axios',
      synopsis: 'Package axios is an ergonomic, axios-style HTTP client built on net/http.',
      doc: 'Package axios is an ergonomic, axios-style HTTP client built on net/http.',
      consts: [],
      vars: [],
      types: [
        {
          ...
          doc: 'Client is a reusable, configured HTTP client.',
          consts: [],
          vars: [],
          funcs: [],
          methods: [],
      ...
      funcs: [{ name: 'New', signature: 'func New(cfg Config) *Client', doc: 'New creates a Client from the given configuration.' }],
    },
  ],
};
...
describe('DocsView', () => {
  ...
  it('renders the inline React API reference from the fetched doc.json', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /package axios/ })).toBeInTheDocument();
    expect(container.querySelector('#sym-New'), 'func New symbol card').not.toBeNull();
    expect(container.querySelector('#sym-Client'), 'type Client symbol card').not.toBeNull();
